...
- [Populate data table](#populate-data-table)
...
One caveat is that we need to start our variables with `REACT_APP_`. You can read more about environmental variables in [create-vue-app's guide](https://facebook.github.io/create-vue-app/docs/adding-custom-environment-variables).
...
##### src/content/RepoPage/LinkList.vue
...
Using the v-if and v-else directives we switch based on the contents of hte cell between the standard rendering and the LinkList component.
...
\_Note: ':prop' is an abrieviation of 'v-bind:prop'
\_Note: '@event' is an abreviation of 'v-on:event'
...
\_Note: We could have passed values for pageSize and page into our pagination component if we had a specific page or page size we wanted to start with. Instead we are relying on the pagination component to set some sensible defaults and provide us with the details through an event.
...
To fix this add the following line to the computed rows property of the RepoPage component you should be able to find something better.

```javascript
console.dir(this.organization.repositories.nodes);
```

\_Hint: id and url are likely unique properties, you could use either of these to update the prototype. In fact we already pass the id value to RepoTable.
