...
See [here](https://github.com/carbon-design-system/carbon-components-vue/blob/master/packages/core/README.md#using-the-components-directly-or-individually) for other ways to load components from @carbon/vue components.
...
Now as a lazy programmer I like to take short cuts in my day, you should too. If you are using VSCode then you might want to add the following snippet which will when you type 'index' generate an index file for you based on the folder name.
...
I also use the following to create a skeleton component of the form I typically use. To use this one start typing the word teamplate in your template file and it will generate a file maiking assumptions based on the compoent file name.
...
Now let's import the icons from our `@carbon/icons-vue` elements package. At the bottom of the `TutorialHeader.vue` file, we need to import each individual icon we will use.
...
Awesome! We've just created our content pages. Next thing we need to do is render them with our router, which will also fix the router error you can see now about the component for the `about` route not being found.
