...
In the `src` directory, create a sub-directory `styles` and add the file `_carbon.scss` to it. Then in `App.vue` edit the style tag to import it.
...
_Note: The landing page is loaded immediately, with the RepoPage lazy loaded. This is one simple way to create webpackChunks and is the default suggestion from the CLI plugin for vue-router._
