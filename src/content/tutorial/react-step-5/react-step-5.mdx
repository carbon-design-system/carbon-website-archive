...
With the Cloud Foundry CLI installed, next we need to create a `manifest.yml` file in the root of the project. You'll need to add your GitHub personal access token here. If you haven't created a GitHub access token yet, see [step 3](/tutorial/react-step-3#create-access-token).

To prevent muliple apps trying to use the `carbon-tutorial` name, replace `USERNAME` with your GitHub username below to make sure our apps are uniquely named.
