...
_Note: You'll notice that these 32px icons aren't the pictograms as designed. The Carbon team is currently working on adding pictograms to the icons packages. Until then, the biggest SVGs._
...
Our `InfoCard` headings looks to be too small, to increase their font sizes according to the design spec with:
...
But, those 3 other violations came from the `<article>` element used in new `InfoCard`. Since the `<article>` element requires a label to be accessible, it seems like we may be using the wrong semantic element. A humble `<div>` will suffice.
