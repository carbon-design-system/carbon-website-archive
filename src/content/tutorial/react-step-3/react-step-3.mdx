...
At this point, let's use the `Query` component that we've imported to run the query and `console.log()` the results. Wrap the `RepoTable` with the `Query` component that passes in the `REPO_QUERY` through the `query` prop.
...
Now that we have that data, let's populate the data table. Replace `console.log(organization);` with the following that destructures the `organization` object because we're only interested in `organization.repositories`. Once we have the respositories, we can call our `getRowItems()` helper to build the data table rows.
...
We need to modify one apsect of the data table because if you expand a row, it says `Row description`. We want to update that with the repository description coming from the GitHub API. To do so, in `RepoTable.js`, add a helper function as the first lines inside the `RepoTable`. This should go immediately before the component's `return()`.
...
Finally, let's add the `Pagination` to updates our state variables and causes the data table to render new rows.
