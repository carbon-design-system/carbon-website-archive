...
Now that we have our repository set up, let's checkout the branch for this tutorial step's starting point. Run the two commands:
...
To optimize our Sass compile time, we'll be adding app-specific styling to `app.scss` and only modifying `index.scss` when necessary.
...
At this point you can switch between the landing and repo pages, but there is still a hard page reload in doing so. We need to do a couple quick fixes to the UI Shell to have it work better with React router.
...
We have a `ci-check` script defined in `package.json` that does a few things. First, it formats files that have been touched since the last Git commit with a tool called Prettier. Then, it runs the `react-scripts test` command that uses Jest under the hood. Run that command:

```bash
$ yarn ci-check
```

Make sure that all tests pass by selecting `a` in Jest's interactive watch mode and once all tests pass, exit the command-line interface with `q`.

**TODO**: add instructions to update snapshots if they are failing.

### Git commit

After everything looks good, stage and commit all of your changes with:
...
Finally, visit [carbon-tutorial](https://github.com/carbon-design-system/carbon-tutorial) to "Compare & pull request". In doing so, make sure that you are comparing to `react-step-1` into `base: react-step-1`.

**TODO**: screenshots, PR name, body, how CI checks work, how Netlify works, submitting PR.
