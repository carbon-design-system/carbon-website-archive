...
Please note that upgrading to the v10 kit will not automatically change any files created in previous versions of the kit. In Sketch, the migration of existing v9 layouts will be a manual process for the designer; there is currently no simple way to automatically update components in a Sketch file from one version of the kit to another. Also note that even though libraries from multiple versions of Carbon can exist at the same time in Sketch, **using v10 components in the same layout as pre-v10 components is not an approved use of the design system.**
...
(Optional) To open the design kit as a Sketch file, click on the "eye" icon in the Preferences panel. Then click on "Open with Sketch." You can use this file for visual reference and guidance while working with the kit.
...
To access the IBM Design Language icons, pictograms, and colors, [subscribe to the IBM Design Language Kit via Sketch Cloud](sketch://add-library/cloud/75VZZ), using the same method you used for the Carbon Design Kit.
...
See [themes guidelines](/guidelines/themes) for detailed info on using themes in Carbon v10.
...
The page grid has been completely redesigned for v10, to align with the IBM Design Language. The official IBM grid is now 16 columns. However, to make the migration process for v9 products smoother, Carbon v10 will default to the 12-column grid, with an option to switch to the 16-column grid. 
...
Migrating your product to Carbon v10 may be daunting, but it doesn't have to be difficult. 
