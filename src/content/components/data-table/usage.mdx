...
Search returns results based on matches. It is used to locate and display a set of results. Search functionality within a table follows [small search](/components/search) conventions.
...
Improve table usability by making data sortable. Clickable headers enable users to sort table data up or down. The specific attribute being sorted is accompanied by an arrow icon. The arrow will only appear on the column headers once that particular column is being sorted.
...
![Inline actions on data tables](images/data-table-usage-8.png)
